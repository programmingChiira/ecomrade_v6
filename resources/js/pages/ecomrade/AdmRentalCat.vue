      ...
      <div v-if="type == 'admin'" class="container">
        ...
        <div class="row justify-content-center my-2 py-1">
          <div class="custom-search">
            <input type="text" class="custom-search-input" placeholder="Search rental | Item | Category...">
          </div>
        </div>
        ...
        <div class="row">
            ...
            <nav aria-label="breadcrumb" class="breadcrumb-nav mb-2">
              <div class="container">
                <ol class="breadcrumb">
                  ...
                  <li style="color: #189483;" class="breadcrumb-item"><strong>Rental Cat</strong></li>
                </ol>
              </div>
            </nav>
          </div>
          ...
          <div style="float:right;" class="col-md-6 col-2">
            <router-link :to="{ name: 'CreateRentalCategory' }" style="float: right;">
            ...
            <div class="">
              <div class="table-responsive">
                <table class="table project-list-table table-nowrap align-middle table-borderless">
                  ...
                  <tbody>
                    <tr v-for="(category, index) in rentalcategories" :key="category.id">
                      ...
                      <td v-if="id == 1">
                        <router-link class="edit-link"
                          :to="{ name: 'PolishRentalCategory', params: { id: category.id } }">
                ...
                <h5 v-if="!rentalcategories.length">Sorry, no item found!</h5>
              </div>
            </div>
          </div>
        </div>
  ...
  methods: {
    destroy(id) {
      Swal.fire({
        ...
        if (result.value) {
          axios
            .delete("/api/rentalcategories/" + id)
            .then((response) => {
              this.success = true;
              Swal.fire(
                'Deleted!',
    ...
    fetchCategories() {
      axios
        .get("/api/rentalcategories")
        .then((response) => (this.rentalcategories = response.data))
        .catch((error) => {
          console.log(error);
        });
    },
